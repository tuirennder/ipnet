...
def create_python_info_panel(console: Console) -> Panel:
    ...
    if env_info["in_virtual_env"]:
        ...
        info_table.add_row("Environment:", f"[bold {env_style}]{env_info['environment_type']}[/]")
        if env_info["environment_path"]:
            info_table.add_row("Environment Path:", f"[dim]{env_info['environment_path']}[/]")
    else:
...
def main_callback(ctx: typer.Context) -> None:
    ...
    if ctx.invoked_subcommand is None:
        ...
        info_text = Text()
        info_text.append("🌐 IPv4 and IPv6 Subnet Calculator\n\n", style="bold cyan")
